...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.patch(`${import.meta.env.VITE_API_URL}/users/me`, updateData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setProfileData(response.data);
      setPassword('');
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-slate-900 flex flex-col items-center p-4 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg w-full max-w-2xl mt-20">
        {!profileData ? (
          <div className="text-center text-gray-800 dark:text-gray-200">Carregando perfil...</div>
        ) : (
          <>
            ...
            <div className="bg-slate-100 dark:bg-slate-700 p-6 rounded-lg mb-8">
              ...
              <p className="mb-2 text-gray-700 dark:text-gray-300"><span className="font-bold">Membro desde:</span> {new Date(profileData.createdAt).toLocaleDateString('pt-BR')}</p>
              <p className="text-gray-700 dark:text-gray-300"><span className="font-bold">Tipo de Conta:</span> {profileData.provider}</p>
            </div>
